...
![Navisworks do MCP điều khiển: tìm tấm panel tường chắn theo tên, tô màu cam và
zoom tới](docs/demo.png)

*Toàn bộ ảnh trên không có một cú click nào trong Navisworks:
`find_objects("PANEL")` → 375 kết quả, `set_color(...)` cho 20 tấm,
...
ruff check .        # lint
pytest              # 62 test, KHÔNG cần Navisworks được cài
```

`tests/` chạy được trên máy không có Navisworks vì client kết nối lazy — việc đăng ký
tool không chạm vào COM:

| File | Kiểm cái gì | Cần Windows |
|---|---|---|
| `test_server.py` | Chuỗi chỉ mục, chuyển đổi giá trị, đọc member COM, kiểm tra file, hợp đồng `safe()` | một phần |
| `test_tool_contracts.py` | Bề mặt tool mà model nhìn thấy: số lượng, mô tả, JSON schema | có |
| `test_repo_layout.py` | Tài liệu không lệch code — bảng tool trong hai README phải khớp `server.py` | không |

Phần cần Navisworks thật nằm ở `test_connection.py`, chạy thủ công.

Đóng góp: xem [CONTRIBUTING.md](CONTRIBUTING.md). Mô hình tin cậy (server này giao
quyền điều khiển Navisworks cho AI): xem [SECURITY.md](SECURITY.md). Lịch sử phiên
bản: [CHANGELOG.md](CHANGELOG.md).
